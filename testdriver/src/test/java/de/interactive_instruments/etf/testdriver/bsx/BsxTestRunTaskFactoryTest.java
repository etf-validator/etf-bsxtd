...
public class BsxTestRunTaskFactoryTest {
    ...
    enum TestObjectType {
        ...
        public String getId() {
            switch (this.name()) {
            case "csv":
                return "213c68ef-c603-47b9-bf63-444e5dd92976";
            case "shapefile":
                return "f91277ec-bbd9-49da-88ff-7b494f1f558d";
            }
            return "e1d4a306-7a78-4a3b-ae2d-cf5f0810853e";
        }
    }

    private TestRunDto createTestRunDtoForProject(final TestObjectType testObjectType)
        ...
        testObjectDto.setLabel("Test Object");

        testObjectDto.setTestObjectType(
                (TestObjectTypeDto) TestObjectTypeDetectorManager.getTypes(testObjectType.getId()).asCollection().iterator()
                        .next());

        testObjectDto.addResource(
                new ResourceDto("testData", new IFile("./build/resources/test/testdata/" + testObjectType.toString()).toURI()));
    ...
    public static void setUp()
        ...
        if (testDriverManager == null) {
            ...
            lc.getItemRegistry().register(TestObjectTypeDetectorManager.getSupportedTypes().asCollection());
    ...
    // @Test
    public void runGmlTest() throws Exception, ComponentNotLoadedException {

        // DO NOT RUN THE TESTS IN THE IDE BUT DIRECTLY WITH GRADLE

        final TestRunDto testRunDto = createTestRunDtoForProject(TestObjectType.gml);
        ...
        // for (final TranslationArgumentCollectionDto arg :
        // result.getTestModuleResults().get(0).getTestCaseResults().get(0).getTestStepResults().get(0).getTestAssertionResults().get(1).getMessages())
        // {
        // if (arg.getTokenValues().get("text") != null && arg.getTokenValues().get("text").getValue().contains("Cannot parse
        // 'gml:posList': contains 14 values, but coordinate dimension is 3.")) {
    ...
    // @Test
    public void runCsvTest() throws Exception, ComponentNotLoadedException {

        // DO NOT RUN THE TESTS IN THE IDE BUT DIRECTLY WITH GRADLE

        final TestRunDto testRunDto = createTestRunDtoForProject(TestObjectType.csv);
    ...
    // @Test
    public void runShapeTest() throws Exception, ComponentNotLoadedException {

        // DO NOT RUN THE TESTS IN THE IDE BUT DIRECTLY WITH GRADLE

        final TestRunDto testRunDto = createTestRunDtoForProject(TestObjectType.shapefile);
