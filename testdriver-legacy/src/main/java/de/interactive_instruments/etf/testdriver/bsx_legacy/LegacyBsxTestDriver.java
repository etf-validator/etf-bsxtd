...
import static de.interactive_instruments.etf.testdriver.bsx_legacy.LegacyBsxTestDriver.BSX_TEST_DRIVER_EID;
import static de.interactive_instruments.etf.testdriver.bsx_legacy.LegacyTypes.BSX_SUPPORTED_TEST_OBJECT_TYPES;
...
public class LegacyBsxTestDriver extends AbstractTestDriver {
    ...
    private final ComponentInfo info = new ComponentInfo() {
        ...
        public String getDescription() {
            return "Legacy test driver for BaseX " + BaseX.class.getPackage().getImplementationVersion();
        }
    };

    public LegacyBsxTestDriver() {
        super(new ConfigProperties(ETF_DATA_STORAGE_NAME, LegacyBsxConstants.PROJECT_DIR_KEY));
    }
    ...
    public TestTask createTestTask(final TestTaskDto testTaskDto) throws TestTaskInitializationException {
        try {
            ...
            return new LegacyBasexTestTask(testTaskDto, ((WriteDao) dataStorageCallback.getDao(TestObjectDto.class)),
                    configProperties.getPropertyOrDefaultAsLong(LegacyBsxConstants.DB_MAX_CHUNK_SIZE, DEFAULT_MAX_CHUNK_SIZE));
    ...
    protected void doInit() throws ConfigurationException, InitializationException, InvalidStateTransitionException {
        ...
        loader = new LegacyBsxFileLoaderFactory(dataStorageCallback);
